...
describe("describeRosterBlock", () => {
  ...
  it("one line per seat, self marked, vision flagged, guidance footer present", () => {
    const block = describeRosterBlock(seats, "planner")
    ...
    expect(block).toContain("Write for the seat you address")
  })

  it("the self marker follows the receiver", () => {
    const block = describeRosterBlock(seats, "tester")
